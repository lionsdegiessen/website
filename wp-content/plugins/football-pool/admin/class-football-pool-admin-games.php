...
class Football_Pool_Admin_Games extends Football_Pool_Admin {
		...
		switch ( $action ) {
			...
			case 'delete':
				...
				if ( $success )
					...
					self::notice( __( 'Error performing the requested action.', 'football-pool' ), 'important' );
			case 'view':
			default:
				self::view();
		}
			...
			if ( count( $header ) == count( $column_names ) ) {
				...
				if ( count( $err ) == 0 ) {
					...
					while ( ( $data = fgetcsv( $fp, 0, FOOTBALLPOOL_CSV_DELIMITER ) ) !== false ) {
						$row++; // start at 1, because first row contains header
	...
	private static function view_schedules( $log = '' ) {
		...
		if ( ! $upload_is_readable && ! $data_is_readable ) {
			...
			self::notice( __( "Please make sure that the directory 'data/schedules' exists in the plugin directory and that it is readable!", 'football-pool' ), 'important' );
			self::notice( __( "Please make sure that the directory 'football-pool/schedules' exists in the WordPress uploads directory and that it is readable!", 'football-pool' ), 'important' );
			return;
		}
		
		// show warnings when one of the dirs is not readable
		if ( ! $data_is_readable ) {
			self::notice( __( "Please make sure that the directory 'data/schedules' exists in the plugin directory and that it is readable!", 'football-pool' ) );
		}
		if ( ! $upload_is_readable ) {
			self::notice( __( "Please make sure that the directory 'football-pool/schedules' exists in the WordPress uploads directory and that it is readable!", 'football-pool' ) );
		}
		
		if ( ! $upload_is_writable ) {
			self::notice( sprintf( __( "Uploading of new csv files is not possible at the moment. Directory '%s' is not writable.", 'football-pool' ), FOOTBALLPOOL_CSV_UPLOAD_DIR ) );
		}
		...
		if ( $upload_is_readable || $data_is_readable ) {
			...
			if ( count( $files ) > 0 ) {
				...
				self::notice( __( "No csv files found in 'upload' directory.", 'football-pool' ) );
			}
		}
